//Chris Auslander
//Project 2
//sft_client_main.c

#include <string.h>
#include <stdio.h>
#include <stdlib.h>
#include <netdb.h>
#include <sys/socket.h>
#include <unistd.h>
#include <netdb.h>
#include <arpa/inet.h>
#include <pthread.h>
#include "sft_client.h"

int main(int argc, char const *argv[])
{
    //Define variables for client
    FILE *fptr;
    pthread_t t1;
    pthread_t t2;
    pthread_t t3;
    char file_to_request[10];
    int port = 1600;

    //Open the requests.txt file
    fptr = fopen("requests.txt", "r");
    if(fptr == NULL){
        printf("file not found\n");
        exit(0);
    }

    //Read all lines - Remove newline from text
    while(fgets(file_to_request, 8, fptr) != NULL){
        if(*(file_to_request+strlen(file_to_request)-1) == '\n'){
            *(file_to_request+strlen(file_to_request)-1) = 0;
        }
        connectiondetails* details = malloc(sizeof(connectiondetails));
        details->address = "127.0.0.1";
        details->file_to_request = file_to_request;
        details->port = port;

        pthread_create(&t1, NULL, make_request, details);
        pthread_create(&t2, NULL, make_request, details);
        pthread_create(&t3, NULL, make_request, details);

        pthread_join(t1,NULL);
        pthread_join(t2,NULL);
        pthread_join(t3,NULL);
    }

    fclose(fptr);
    return 0;
}
